...
async def test_create_read_update_complete_delete(david, world):
    ...
    # stale If-Match -> 412
    resp = await david.patch(
        f"/api/v1/tasks/{tid}", headers={"If-Match": '"99"'}, json={"priority": 1}
    )
    assert resp.status_code == 412

    # correct If-Match -> 200, version bumps
    resp = await david.patch(
        f"/api/v1/tasks/{tid}", headers={"If-Match": '"1"'}, json={"priority": 1}
    )
    ...
    resp = await david.post(
        f"/api/v1/tasks/{tid}/complete", headers={"If-Match": '"2"'}
    )
    assert resp.status_code == 200
...
async def test_subtask_depth_is_capped_at_two(david, world):
    parent = (
        await david.post(
            f"/api/v1/projects/{world['mktg_id']}/tasks", json={"title": "Parent"}
        )
    ).json()
    child = (
        await david.post(
...
async def test_every_mutation_writes_an_audit_event(david, as_user, world):
    ...
    created = await david.post(
        f"/api/v1/projects/{world['mktg_id']}/tasks", json={"title": "Audited task"}
    )
    tid = created.json()["id"]
    await david.patch(
        f"/api/v1/tasks/{tid}", headers={"If-Match": '"1"'}, json={"title": "Audited task v2"}
    )
...
async def test_optimistic_concurrency_blocks_lost_updates(as_user, world):
    ...
    tid = (
        await a.post(f"/api/v1/projects/{world['mktg_id']}/tasks", json={"title": "Shared edit"})
    ).json()["id"]
