...
def _guard_dev_auth() -> None:
    if not settings.dev_auth_enabled:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Dev auth is disabled."
        )
...
async def dev_login(payload: DevLoginRequest, session: SessionDep) -> TokenResponse:
    ...
    token = mint_dev_token(
        entra_object_id=user.entra_object_id, upn=user.upn, display_name=user.display_name
    )
...
async def whoami(user: CurrentUser, session: SessionDep) -> SessionInfo:
    ...
    grants = (
        (await session.execute(select(RoleGrant).where(RoleGrant.user_id == user.id)))
        .scalars()
        .all()
    )
    portfolios = {
        p.id: p.name for p in (await session.execute(select(Portfolio))).scalars().all()
    }
    for g in grants:
        info.roles.append(
            SessionRole(
